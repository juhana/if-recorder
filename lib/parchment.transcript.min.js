parchment.transcript={sessionId:(new Date).getTime()+""+Math.ceil(Math.random()*1E3),command:{input:"",timestamp:0},output:"",statusline:"",window:0,turncount:0,inputcount:0,outputcount:1,styles:"",saveUrl:"",storyUrl:"",optOut:typeof getUrlVars().feedback!="undefined"&&getUrlVars().feedback!="1",serverOffline:false,send:function(b,a,c){if(this.collectTranscripts()){if(typeof b!="undefined")this.window=b;if(typeof a!="undefined")this.styles=a;if(typeof c!="undefined")this.output=c;b=$.toJSON({session:this.sessionId,
log:{inputcount:this.inputcount,outputcount:this.outputcount,input:this.command.input,output:this.output,window:this.window,styles:this.styles,timestamp:(new Date).getTime()}});$.ajax({type:"POST",url:this.saveUrl,data:{data:b}});this.output="";this.outputcount++}},collectTranscripts:function(){if(this.saveUrl==""||this.optOut||this.serverOffline)return false;return true},initialize:function(b){if(typeof b=="string")this.saveUrl=b;if(this.storyUrl=="")this.storyUrl=parchment.options.default_story?
parchment.options.default_story:getUrlVars().story;if(!this.collectTranscripts())return false;b=$.browser.name+" "+$.browser.version+" "+$.os.name;var a="Quixe";if(parchment.lib.Story.zcode)a="Gnusto";b=$.toJSON({session:this.sessionId,start:{story:this.storyUrl,interpreter:"Parchment / "+a,browser:b}});$.ajax({type:"POST",url:this.saveUrl,data:{data:b},error:function(){this.serverOffline=true},success:function(c){this.serverOffline=c.toLowerCase()!="ok"}});return true},charName:function(b){switch(b){case 8:return"<backspace>";
